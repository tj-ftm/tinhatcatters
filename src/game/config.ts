...
export function generatePlaceholderAssets(scene: Phaser.Scene): void {
  ...
  sparkleGraphics.fillStar(12, 12, 5, 10, 5);
  sparkleGraphics.generateTexture('sparkle', 24, 24);
  sparkleGraphics.destroy();
  
  // Background layers
  const bgLayer1Graphics = scene.make.graphics({ x: 0, y: 0, add: false });
  ...
  const bgLayer2Graphics = scene.make.graphics({ x: 0, y: 0, add: false });
  ...
  const bgLayer3Graphics = scene.make.graphics({ x: 0, y: 0, add: false });
  ...
  // TinHatCatter
  const tinHatGraphics = scene.make.graphics({ x: 0, y: 0, add: false });
